...
public class Mediator {
   ...
   */
  public MediatorPanel<?> createBossPanel(int id) { return new MediatorPanel<>(controller.createBossPanel(id));
  }
  ...
  public MediatorWildUnit<?> createWildUnit(String name, int hitPoints, int attack, int defense,
    ...
    return new MediatorWildUnit<>(
        controller.createWildUnit(name, hitPoints, attack, defense, evasion));
  }
  ...
  public MediatorPlayer<?> getWinner() {
    for (PlayerUnit unit : controller.getPlayers()){
      if (unit.winner==1){
        return new MediatorPlayer<>(unit);
      }
    }
    return null;
  }
   ...
   *     The player's class.
   */
  public static class MediatorPlayer<T extends PlayerUnit> extends MediatorUnit<T> {

    public MediatorPlayer(String name, int hitPoints, int attack, int defense, int evasion) {
      super((T) new PlayerUnit(name, hitPoints, attack, defense, evasion));
    }
